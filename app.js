...
function openSectorDetail(s){selectedSector=s;$('#sectorModalTitle').textContent=`${s.name} · ${(s.etfs||[]).join(' / ')}`;const m=s.metrics||{};const ext=(m.session==='premarket'||m.session==='afterhours')?` · ${m.session==='premarket'?'프리':'애프터'} ${Number(m.extended_change_pct||0)>=0?'+':''}${Number(m.extended_change_pct||0).toFixed(2)}% · 보정 ${Number(m.extended_score_adjustment||0)>=0?'+':''}${Number(m.extended_score_adjustment||0).toFixed(1)}점`:'';$('#sectorModalMeta').textContent=`레이더 ${s.score}점 · ${s.status}${ext}`;const f=s.factors||{};const factors=[['가격 모멘텀',f.momentum],['상대강도·개선',f.relative_strength],['거래량 확산',f.volume],['시장 Breadth',f.breadth],['Macro 환경',f.macro]];$('#factorBars').innerHTML=factors.map(([n,v])=>`<div class="factor"><div class="factor-name">${n}</div><div class="factor-bar"><div class="factor-fill" style="width:${Number(v||0)}%"></div></div><div class="factor-score">${v??'-'}/100</div></div>`).join('');$('#sectorDetailReason').textContent=s.reason||'';$('#sectorDetailAction').textContent=`현재 판단: ${s.action}`;$('#sectorQuoteBtn').onclick=()=>openSourceModal(`${s.etfs?.[0]||''} 시세·차트`,s.name,s.quote_sources||[]);$('#sectorModal').classList.add('open');$('#sectorModal').setAttribute('aria-hidden','false');}
