...
function trendClass(label=''){if(label.includes('빠른 개선')||label.includes('완만한 개선'))return 'trend-up';if(label.includes('급격한 약화')||label.includes('↘'))return 'trend-down';return 'trend-flat';}
function confidenceClass(score=0){const n=Number(score);return n>=80?'confidence-a':n>=65?'confidence-b':n>=50?'confidence-c':'confidence-d';}
function rankText(s){const c=s.rank_change;return `#${s.rank??'-'}${c===null||c===undefined?'':c>0?` ↑${c}`:c<0?` ↓${Math.abs(c)}`:' →'}`;}
function renderRadar(d){
  const rows=sectorRows(d);
  $('#sectorTable').innerHTML=rows.map(s=>{const reasons=extractReason(s.reason);const idx=(d.sectors||[]).indexOf(s);const tr=s.trend||{},cf=s.confidence||{};const provisional=s.provisional?'<span class="provisional-tag">잠정</span>':'';return `<tr data-sector-index="${idx}"><td><span class="sector-name">${escapeHtml(s.name)}</span><span class="etf-sub">(${escapeHtml((s.etfs||[]).join(' / '))}) · ${rankText(s)}</span></td><td><span class="chip ${chipClass(s.status)}">${escapeHtml(s.status)}</span>${provisional}</td><td><div class="score-wrap"><span class="score-num">${s.score}</span><span class="score-bar"><span class="score-fill ${scoreFillClass(s.status)}" style="width:${Math.max(0,Math.min(100,s.score))}%"></span></span></div><div class="score-sub">원 ${s.raw_score??s.score} · 3일 ${tr.avg3??'-'}</div></td><td><div class="trend-line ${trendClass(tr.label)}">${escapeHtml(tr.label||'→ 유지')}</div><div class="confidence-line ${confidenceClass(cf.score)}">신뢰 ${cf.score??'-'} · ${cf.grade||'-'} · ${s.signal_days||1}일</div><span class="action ${actionClass(s.action)}">${escapeHtml(s.action)}</span></td><td class="reason-mini">${reasons.map(x=>`<span>• ${escapeHtml(x)}</span>`).join('')}</td><td class="row-arrow">›</td></tr>`}).join('');
  $('#sectorCards').innerHTML=rows.map(s=>{const idx=(d.sectors||[]).indexOf(s),tr=s.trend||{},cf=s.confidence||{};return `<div class="sector-card" data-sector-index="${idx}"><div class="sector-card-top"><div><b>${escapeHtml(s.name)}</b><small>${escapeHtml((s.etfs||[]).join(' / '))} · ${rankText(s)}</small></div><div><span class="chip ${chipClass(s.status)}">${escapeHtml(s.status)}</span>${s.provisional?'<span class="provisional-tag">잠정</span>':''}</div></div><div class="score-wrap mobile-score"><span class="score-num">${s.score}</span><span class="score-bar"><span class="score-fill ${scoreFillClass(s.status)}" style="width:${s.score}%"></span></span></div><div class="action ${actionClass(s.action)}">${escapeHtml(s.action)}</div><div class="mobile-intel"><span class="${trendClass(tr.label)}">${escapeHtml(tr.label||'→ 유지')}</span><span class="${confidenceClass(cf.score)}">신뢰 ${cf.score??'-'} ${cf.grade||''}</span><span>원 ${s.raw_score??s.score} · ${s.signal_days||1}일</span></div><div class="reason-mini">${extractReason(s.reason).map(x=>`<span>• ${escapeHtml(x)}</span>`).join('')}</div></div>`}).join('');
  $$('[data-sector-index]').forEach(el=>el.onclick=()=>openSectorDetail(d.sectors[Number(el.dataset.sectorIndex)]));
}
function openSectorDetail(s){selectedSector=s;$('#sectorModalTitle').textContent=`${s.name} · ${(s.etfs||[]).join(' / ')}`;const m=s.metrics||{},tr=s.trend||{},cf=s.confidence||{},dq=s.data_quality||{},pt=s.participation||{};const ext=(m.session==='premarket'||m.session==='afterhours')?` · ${m.session==='premarket'?'프리':'애프터'} ${Number(m.extended_change_pct||0)>=0?'+':''}${Number(m.extended_change_pct||0).toFixed(2)}% · 보정 ${Number(m.extended_score_adjustment||0)>=0?'+':''}${Number(m.extended_score_adjustment||0).toFixed(1)}점`:'';$('#sectorModalMeta').textContent=`안정화 ${s.score}점 · 원점수 ${s.raw_score??s.score}점 · ${s.status} · ${rankText(s)}${ext}`;const f=s.factors||{};const factors=[['가격 모멘텀',f.momentum],['상대강도·개선',f.relative_strength],['거래량 확산',f.volume],['시장 Breadth',f.breadth],['Macro 환경',f.macro]];$('#factorBars').innerHTML=factors.map(([n,v])=>`<div class="factor"><div class="factor-name">${n}</div><div class="factor-bar"><div class="factor-fill" style="width:${Number(v||0)}%"></div></div><div class="factor-score">${v??'-'}/100</div></div>`).join('');const flags=(s.flags||[]).length?(s.flags||[]).map(x=>`<span class="intel-flag">${escapeHtml(x)}</span>`).join(''):'<span class="intel-ok">특이 경고 없음</span>';$('#signalIntel').innerHTML=`<div><small>최근 점수</small><b>${(tr.series||[]).join(' → ')||'-'}</b></div><div><small>3일 / 5일 평균</small><b>${tr.avg3??'-'} / ${tr.avg5??'-'}</b></div><div><small>추세</small><b class="${trendClass(tr.label)}">${escapeHtml(tr.label||'-')}</b></div><div><small>고점 대비</small><b>${Number(tr.peak_drop_5d||0)>=0?'+':''}${tr.peak_drop_5d??'-'}점</b></div><div><small>신뢰도</small><b class="${confidenceClass(cf.score)}">${cf.score??'-'} · ${cf.grade||'-'}</b></div><div><small>데이터 품질</small><b>${dq.score??'-'} · ${dq.grade||'-'}</b></div><div><small>산업 내부 확산</small><b>${pt.score??'-'}/100</b></div><div><small>SPY 상관 20/60일</small><b>${m.corr20_spy??'-'} / ${m.corr60_spy??'-'}</b></div><div><small>갭 / 20일선 이격</small><b>${m.gap_pct??'-'}% / ${m.distance_ma20_pct??'-'}%</b></div><div><small>신호 지속</small><b>${s.signal_days||1}일</b></div><div class="intel-flags"><small>품질 경고</small>${flags}</div>`;$('#sectorDetailReason').textContent=s.reason||'';$('#sectorDetailAction').textContent=`현재 판단: ${s.action}${s.provisional?' (장외 잠정)':''}`;$('#sectorQuoteBtn').onclick=()=>openSourceModal(`${s.etfs?.[0]||''} 시세·차트`,s.name,s.quote_sources||[]);$('#sectorModal').classList.add('open');$('#sectorModal').setAttribute('aria-hidden','false');}
